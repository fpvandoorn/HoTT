...
Require Import HoTT.Basics Types.Prod Types.ΠTypes.Paths.
...
/- Given using the variable names from [transport : Π{A : Type} (P : A → Type) {x y : A}, x = y → P x → P y] and [path_pi : {funext} → Π{A B} (f g : Πx : A, B x), (Πx : A, f x = g x) → f = g]:

The high-level idea is that we don't really need functional extensionality if we end up just applying the functions to arguments anyway.  That is, if we have that [Πx, f x = g x], and we only talk about [f y] and [f z], then we don't actually need to transport across [f = g], just [f y = g y] and [f z = g z].

In a bit more detail, if we are transporting across [path_pi f g H], and in the function [P], all instances of [f] are applied to some expressions, say we only see [f x], [f y], ..., [f z], then we can eliminate the [path_forall] by explicitly transporting across [H x], [H y], ..., [H z].  The lemma [path_pi_1_beta] expresses this fact in the case that we see [f] applied to only a single argument in [P], and the tactic [transport_path_pi_hammer] is some fancy Ltac to auto-infer what [P] is and what the argument to [f] is.

The way the tactic does this is by creating an evar for [P] and an evar for the argument to [f], and then using a combination of [assert], [pattern], etc to figure out what each should be.  If you want to see how it works, you can step through each step of [transport_path_pi_hammer] when trying to prove [path_pi_2_beta]. -/

/- First, we prove some helpful lemmas about [path_forall] and [transport] -/
Local Ltac path_pi_beta_t :=
  lazymatch goal with
    | [ |- context[@path_pi ?H ?A ?B ?f ?g ?e] ]
      => let X := fresh in
         pose proof (sect (@path_pi H A B f g) e) as X;
           case X;
           generalize (@path_pi H A B f g e);
           clear X; clear e;
           intro X; destruct X;
           simpl;
           unfold apD10;
...
definition path_pi_1_beta [H : funext] A B x P f g e Px
: @transport (Πa : A, B a) (λf, P (f x)) f g (@path_pi _ _ _ _ _ e) Px
...
definition path_pi_recr_beta' [H : funext] A B x0 P f g e Px
: @transport (Πa : A, B a)
             (λf, P f (f x0))
             f
             g
  ...
  = @transport ((Πa, B a) × B x0)%type
               ...
               (f, f x0)
               (g, g x0)
               (path_prod' (@path_pi _ _ _ _ _ e) (e x0))
               Px.
/-begin
  path_pi_beta_t.
end-/
...
Ltac infer_path_pi_recr_beta term :=
  let path_pi_recr_beta' :=
      match term with
        | @transport _ (λx, @?P0 x) _ _ (@path_pi ?H ?A ?B ?f ?g ?e) _
          => constr:(λx0 P Px, @path_pi_recr_beta H A B x0 P f g e Px)
      end in
  let Px := match term with @transport _ _ _ _ _ ?Px => constr:(Px) end in
  let P0 := match term with @transport _ (λf, @?P0 f) _ _ _ _ => constr:(P0) end in
  ...
  let P0f := constr:(λWORKAROUND_FOR_BUG_3458, $(let ret := pull_app (P0 WORKAROUND_FOR_BUG_3458) WORKAROUND_FOR_BUG_3458 in
                                                      exact ret)$) in
  let x0 := match P0f with λf, (?x0, @?P f) => constr:(x0) end in
  let P := match P0f with λf, (?x0, @?P f) => constr:(P) end in
  let ret := constr:(path_pi_recr_beta' x0 P Px) in
...
Ltac transport_path_pi_hammer_helper :=
  let term := match goal with
                | |- context[@transport ?At (λx, @?Bt x) ?ft ?gt (@path_pi ?H ?A ?B ?f ?g ?e) ?Px]
                  => constr:(@transport At Bt ft gt (@path_pi H A B f g e) Px)
              end in
  let lem := infer_path_pi_recr_beta term in
  pattern term;
    refine (transport _ lem⁻¹ _);
    cbv beta.

Ltac transport_path_pi_hammer :=
  transport_path_pi_hammer_helper;
...
definition path_pi_2_beta' [H : funext] A B x0 x1 P f g e Px
: @transport (Πa : A, B a) (λf, P (f x0) (f x1)) f g (@path_pi _ _ _ _ _ e) Px
...
/- Takes a term of type [_ = _], and tries to replace it by [refl] by trying to prove that it's an hProp.  The ordering of attempts is tuned for speed. -/
Ltac clear_contr_path p :=
  let H := fresh in
  let T := type of p in
  progress (
      first [ assert (H : refl = p) by exact (center _)
            | assert (H : refl = p)
              by (
            ...
            | pose proof (@path_contr T _ refl p) as H ];
      destruct H;
      /- now reduce any matches on [refl] (and on other things too) -/
      cbv iota in *
    ).
...
Ltac step_clear_paths_in_match :=
  idtac;
  match goal with
    | [ |- context[match ?p with refl => _ end] ] => progress destruct p
    | [ |- context[match ?p with refl => _ end] ] => clear_path_no_check p
  end.
...
Ltac rewrite_hyp' := do_with_hyp ltac:(λH, rewrite H).
Ltac rewrite_hyp := repeat rewrite_hyp'.
Ltac rewrite_rev_hyp' := do_with_hyp ltac:(λH, rewrite <- H).
Ltac rewrite_rev_hyp := repeat rewrite_rev_hyp'.

Ltac apply_hyp' := do_with_hyp ltac:(λH, apply H).
Ltac apply_hyp := repeat apply_hyp'.
Ltac eapply_hyp' := do_with_hyp ltac:(λH, eapply H).
...
Tactic Notation "simpl" "rewrite"      open_constr(term) := simpl_do_clear ltac:(λH, rewrite    H) term.
Tactic Notation "simpl" "rewrite" "->" open_constr(term) := simpl_do_clear ltac:(λH, rewrite → H) term.
Tactic Notation "simpl" "rewrite" "<-" open_constr(term) := simpl_do_clear ltac:(λH, rewrite <- H) term.

Tactic Notation "simpl" "rewrite"      open_constr(term) "in" hyp(hyp) := simpl_do_clear ltac:(λH, rewrite    H in hyp) term.
Tactic Notation "simpl" "rewrite" "->" open_constr(term) "in" hyp(hyp) := simpl_do_clear ltac:(λH, rewrite → H in hyp) term.
Tactic Notation "simpl" "rewrite" "<-" open_constr(term) "in" hyp(hyp) := simpl_do_clear ltac:(λH, rewrite <- H in hyp) term.

Tactic Notation "simpl" "rewrite"      open_constr(term) "in" "*" := simpl_do_clear ltac:(λH, rewrite    H in × ) term.
Tactic Notation "simpl" "rewrite" "->" open_constr(term) "in" "*" := simpl_do_clear ltac:(λH, rewrite → H in × ) term.
Tactic Notation "simpl" "rewrite" "<-" open_constr(term) "in" "*" := simpl_do_clear ltac:(λH, rewrite <- H in × ) term.
