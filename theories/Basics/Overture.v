...
definition paths_ind' {A : Type} (P : Π(a b : A), (a ≈ b) → Type)
  : (Π(a : A), P a a idpath) → Π(a b : A) (p : a ≈ b), P a b p.
Proof.
  intros H ? ? [].
  apply H.
end-/
...
Class IsEquiv {A B : Type} (f : A → B) := BuildIsEquiv {
  ...
  eisadj : Πx : A, eisretr (f x) ≈ ap f (eissect x)
}.
...
Axiom isequiv_apD10 : Π[H : Funext] (A : Type) (P : A → Type) f g, IsEquiv (@apD10 A P f g).
Global Existing Instance isequiv_apD10.

definition path_Π[H : Funext] {A : Type} {P : A → Type} (f g : Πx : A, P x) :
  f == g → f ≈ g :=
...
definition path_forall2 [H : Funext] {A B : Type} {P : A → B → Type} (f g : Πx y, P x y) :
  ...
  (λE, path_Πf g (λx, path_Π(f x) (g x) (E x))).
