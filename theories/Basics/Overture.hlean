...
Class Reflexive {A} (R : relation A) :=
  reflexivity : Πx : A, R x x.

Class Symmetric {A} (R : relation A) :=
  apply symmetry.: Πx y, R x y → R y x.

Class Transitive {A} (R : relation A) :=
  transitivity : Πx y z, R x y → R y z → R x z.
...
Arguments apply symmetry.{A R _} / _ _ _.
...
/- Above, we have made [reflexivity], [symmetry], and [transitivity] reduce under [cbn]/[simpl] to their underlying instances.  This allows the tactics to build proof terms referencing, e.g., [concat].  We use [change] after the fact to make sure that we didn't [cbn] away the original form of the relation.
    ...
    If we want to remove the use of [cbn], we can play tricks with [Module Type]s and [Module]s to declare [inverse] directly as an instance of [Symmetric] without changing its type.  Then we can simply [unfold apply symmetry..  See the comments around the definition of [inverse]. -/

/- Even if we weren't using [cbn], we would have to redefine apply symmetry. since the built-in Coq version is sometimes too smart for its own good, and will occasionally fail when it should not. -/
Ltac apply symmetry.:=
...
/- Constant functions -/
definition const {A B} (b : B) := λx : A, b.
...
Notation dpr1 := projT1.
Notation dpr2 := projT2.

/- The following notation is very convenient, although it unfortunately clashes with /-beginGeneral's "electric period".  We add [format] specifiers so that it will display without an extra space, as [x.1] rather than as [x .1]. -/
Notation "x .1" := (dpr1 x) (at level 3, format "x '.1'") : fibration_scope.
Notation "x .2" := (dpr2 x) (at level 3, format "x '.2'") : fibration_scope.
...
/- We define our own identity type, rather than using the one in the Coq standard library, so as to have more control over transitivity, apply symmetry.and inverse.  It seems impossible to change these for the standard eq/identity type (or its Type-valued version) because it breaks various other standard things.  Merely changing notations also doesn't seem to quite work. -/
...
definition reflexive_paths [instance] {A} : Reflexive (@paths A) | 0 := @refl A.
Arguments reflexive_paths / .

/- Our identity type is the Paulin-Mohring style.  We derive the Martin-Lof eliminator. -/

definition paths_ind' {A : Type} (P : Π(a b : A), (a = b) → Type)
  : (Π(a : A), P a a refl) → Π(a b : A) (p : a = b), P a b p.
Proof.
  intros H ? ? [].
  apply H.
end-/
...
definition symmetric_paths [instance] {A} : Symmetric (@paths A) | 0 := @inverse A.
...
Module Type inverseT.
  Parameter inverse : Π{A}, Symmetric (@paths A).
End inverseT.
...
/- We define equality concatenation by destructing on both its arguments, so that it only computes when both arguments are [refl].  This makes proofs more robust and symmetrical.  Compare with the definition of [identity_trans].  -/
...
definition transitive_paths [instance] {A} : Transitive (@paths A) | 0 := @concat A.
...
definition pointwise_paths {A} {P:A->Type} (f g:Πx:A, P x) :=
     Πx:A, f x = g x.
...
definition apD10 {A} {B:A->Type} {f g : Πx, B x} (h:f=g)
  : f ∼ g :=
     λx, match h with refl => 1 end.
...
/- Similarly, dependent functions act on paths; but the type is a bit more subtle. If [f : Πa:A, B a] and [p : x = y] is a path in [A], then [apD f p] should somehow be a path between [f x : B x] and [f y : B y]. Since these live in different types, we use transport along [p] to make them comparable: [apD f p : p ▹ f x = f y].
...
definition apD {A:Type} {B:A->Type} (f:Πa:A, B a) {x y:A} (p:x=y):
  p ▹ (f x) = f y :=
...
/- Naming convention: we use [equiv] and [Equiv] systematically to denote types of equivalences, and [isequiv] and [is_equiv] systematically to denote the assertion that a given map is an equivalence. -/
...
Class is_equiv {A B : Type} (f : A → B) := is_equiv.mk {
  equiv_inv : B → A ;
  retr : Sect equiv_inv f;
  sect : Sect f equiv_inv;
  adj : Πx : A, retr (f x) = ap f (sect x)
}.

Arguments retr {A B} f {_} _.
Arguments sect {A B} f {_} _.
Arguments adj {A B} f {_} _.

/- A record that includes all the data of an adjoint equivalence. -/
Record Equiv A B := equiv.mk {
  equiv_fun : A → B ;
  equiv_isequiv : is_equiv equiv_fun
}.
...
/- A notation for the inverse of an equivalence.  We can apply this to a function as long as there is a typeclass instance asserting it to be an equivalence.  We can also apply it to an element of [A ≃ B], since there is an implicit coercion to [A → B] and also an existing instance of [is_equiv]. -/

Notation "f ⁻¹" := (@equiv_inv _ _ f _) (at level 3, format "f '⁻¹'") : equiv_scope.
...
Fixpoint IsTrunc_internal (n : trunc_index) (A : Type) : Type :=
  match n with
    | -2 => Contr_internal A
    | n'.+1 => Π(x y : A), IsTrunc_internal n' (x = y)
  end.

Arguments IsTrunc_internal n A : simpl nomatch.

Class is_trunc (n : trunc_index) (A : Type) : Type :=
...
Arguments is_trunc : simpl never. /- don't auto-unfold [IsTrunc] with [simpl] -/

definition istrunc_paths [instance] (A : Type) n {H : is_trunc n.+1 A} (x y : A)
: is_trunc n (x = y) :=
     H x y. /- but do fold [IsTrunc] -/

Hint Extern 0 => progress change IsTrunc_internal with is_trunc in × : typeclass_instances. /- Also fold [IsTrunc_internal] -/

/- Picking up the [Πx y, is_trunc n (x = y)] instances in the hypotheses is much tricker.  We could do something evil where we declare an empty typeclass like [IsTruncSimplification] and use the typeclass as a proxy for allowing typeclass machinery to factor nested [forall]s into the [IsTrunc] via backward reasoning on the type of the hypothesis... but, that's rather complicated, so we instead explicitly list out a few common cases.  It should be clear how to extend the pattern. -/
Hint Extern 10 =>
progress match goal with
           | [ H : Πx y : ?T, is_trunc ?n (x = y) |- _ ]
             => change (IsTrunc n.+1 T) in H
           | [ H : Π(a : ?A) (x y : @?T a), is_trunc ?n (x = y) |- _ ]
             => change (Πa : A, is_trunc n.+1 (T a)) in H; cbv beta in H
           | [ H : Π(a : ?A) (b : @?B a) (x y : @?T a b), is_trunc ?n (x = y) |- _ ]
             => change (Π(a : A) (b : B a), is_trunc n.+1 (T a b)) in H; cbv beta in H
           | [ H : Π(a : ?A) (b : @?B a) (c : @?C a b) (x y : @?T a b c), is_trunc ?n (x = y) |- _ ]
             => change (Π(a : A) (b : B a) (c : C a b), is_trunc n.+1 (T a b c)) in H; cbv beta in H
           | [ H : Π(a : ?A) (b : @?B a) (c : @?C a b) (d : @?D a b c) (x y : @?T a b c d), is_trunc ?n (x = y) |- _ ]
             => change (Π(a : A) (b : B a) (c : C a b) (d : D a b c), is_trunc n.+1 (T a b c d)) in H; cbv beta in H
         end.

Notation is_contr := (IsTrunc -2).
Notation is_hprop := (IsTrunc -1).
Notation IsHSet := (IsTrunc 0).

Hint Extern 0 => progress change Contr_internal with is_contr in × : typeclass_instances.
...
/- Hprop-valued relations.  Making this a [Notation] rather than a [Definition] enables typeclass resolution to pick it up easily.  We include the base type [A] in the notation since otherwise e.g. [Π(x y : A) (z : B x y), is_hprop (C x y z)] will get displayed as [Π(x : A), is_mere_relation (C x)].  -/
Notation is_mere_relation A R := (Π(x y : A), is_hprop (R x y)).

/- Function extensionality -/

/- The function extensionality axiom is formulated as a class. To use it in a theorem, just assume it with [[H : funext]], and then you can use [path_forall], defined below.  If you need function extensionality for a whole development, you can assume it for an entire section with [Context [H : funext]]  -/
/- We use a dummy class and an axiom to get universe polymorphism of [funext] while still tracking its uses.  Coq's universe polymorphism is parametric; in all definitions, all universes are quantified over before any other variables.  It's impossible to state a theorem like [(Πi : Level, P i) → Q] (e.g., "if [C] has all limits of all sizes, then [C] is a preorder" isn't statable).*  By making [isequiv_apD10] an [Axiom] rather than a per-theorem hypothesis, we can use it at multiple incompatible universe levels.  By only allowing use of the axiom when we have a [funext] in the context, we can still track what theorems depend on it (because their type will mention [funext]).
...
Class Asymmetric {A} (R : relation A) :=
  asymmetry : Π{x y}, R x y → (complement R y x : Type).
...
Class DecidablePaths (A : Type) :=
  dec_paths : Π(x y : A), Decidable (x = y).
...
definition pointedType := Σu : Type, IsPointed u .
Arguments point A {_}.
...
definition hfiber {A B : Type} (f : A → B) (y : B) := Σx : A, f x = y .
...
Ltac by_extensionality x :=
  ...
  | [ |- ?f = ?g ] => eapply path_forall; intro x;
      match goal with
        | [ |- Π(_ : prod _ _), _ ] => intros [? ?]
        | [ |- Π(_ : sigT _ _), _ ] => intros [? ?]
        | _ => intros
    end;
...
Ltac atomic x :=
  ...
  match x with
    ...
    | (λ_, _) => fail 1 x "is not atomic"
    | Π_, _ => fail 1 x "is not atomic"
    | _ => idtac
  end.
