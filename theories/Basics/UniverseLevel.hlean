...
definition isequiv_lift [instance] T : is_equiv (@lift T) :=
     @is_equiv.mk
       _ _
       (@lift T)
       (@lower T)
...
definition isequiv_lift' [instance] T : is_equiv (@lift'@{i j} T) :=
     @is_equiv.mk
       _ _
       (@lift' T)
       (@lower' T)
...
/- We make [lift] and [lower] opaque so that typeclass resolution doesn't pick up [isequiv_lift] as an instance of [is_equiv idmap] and wreck havok. -/
