...
section Extensions
  ...
  /-begin
    ...
    apply (functor_sigma (path_pi (ext .1) (ext' .1))). intros p.
    apply (compose (path_pi _ _)). unfold pointwise_paths.
    apply (functor_pi idmap). intros x.
    apply (compose (B := (p (f x))⁻¹ ⬝ (ext .2 x) = (ext' .2 x))).
    apply concat.
    transitivity ((apD10 (path_pi _ _ p) (f x))⁻¹ ⬝ ext .2 x).
  ...
  definition isequiv_path_extension [instance] [H : funext] {A B : Type} {f : A → B}
         {P : B → Type} {d : Πx:A, P (f x)}
         (ext ext' : ExtensionAlong f P d)
  : is_equiv (path_extension ext ext') | 0.
  /-begin
  ...
  /-begin
    ...
    - refine (functor_pi (functor_pi idmap
                                             (λa, (g (f a))⁻¹)) _);
      intros h; simpl.
      refine (functor_sigma (functor_pi idmap g) _); intros k.
      refine (functor_pi idmap _);
        intros a; unfold functor_arrow, functor_forall, composeD; simpl.
      apply moveR_equiv_M.
    - refine (functor_pi (functor_pi idmap (λb, (g b)⁻¹)) _);
      intros h.
      refine (functor_pi (functor_pi idmap (λb, (g b)⁻¹)) _);
  ...
  definition extendable_postcompose (n : nat)
             {A B : Type} (C D : B → Type) (f : A → B)
             (g : Πb, C b → D b) [H : Πb, is_equiv (g b)]
  : ExtendableAlong n f C → ExtendableAlong n f D :=
  ...
  definition extendable_equiv (n : nat)
             {A B : Type} (C : B → Type) (f : A → B) [H : is_equiv _ _ f]
  : ExtendableAlong n f C.
  /-begin
    revert C; simple_induction n n IHn; intros C; [ exact star | split ].
    - intros h.
      exists (λb, retr f b ▹ h (f⁻¹ b)); intros a.
      refine (transport2 C (adj f a) _ ⬝ _).
      refine ((transport_compose C f _ _)⁻¹ ⬝ _).
      exact (apD h (sect f a)).
    - intros h k.
      apply IHn.
  end-/
  ...
  definition isequiv_ooextendable [H : funext]
             {A B : Type} (C : B → Type) (f : A → B)
  : ooExtendableAlong f C → is_equiv (λg, g oD f) :=
