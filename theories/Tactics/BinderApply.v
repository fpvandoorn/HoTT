...
: f ≈ g.
Proof.
  /- We need to apply [path_forall] under binders five times in [H'].  We use a different variant each time to demonstrate the various ways of using this tactic.  In a normal proof, you'd probably just do [do 4 binder apply (@path_Π_) in H'] or just [repeat binder apply (@path_Π_) in H']. -/
  /- If we do [binder apply path_Πin H'], we are told that Coq can't infer the argument [A] to [path_forall].  Instead, we can [binder eapply] it, to tell Coq to defer inference and use an evar for now. -/
  Fail binder apply path_Πin H'.
  binder eapply path_Πin H'.
...
(Funext ->
 ...
 let H0 := H' in Funext). *)
  binder apply @path_Πin H' using assumption.
  binder apply @path_Πin H' using typeclasses eauto.
  binder apply (@path_Π_) in H'.
