...
/- Given using the variable names from [transport : Π{A : Type} (P : A → Type) {x y : A}, x ≈ y → P x → P y] and [path_Π: {Funext} → Π{A B} (f g : Πx : A, B x), (Πx : A, f x ≈ g x) → f ≈ g]:
...
In a bit more detail, if we are transporting across [path_Πf g H], and in the function [P], all instances of [f] are applied to some expressions, say we only see [f x], [f y], ..., [f z], then we can eliminate the [path_forall] by explicitly transporting across [H x], [H y], ..., [H z].  The lemma [path_forall_1_beta] expresses this fact in the case that we see [f] applied to only a single argument in [P], and the tactic [transport_path_forall_hammer] is some fancy Ltac to auto-infer what [P] is and what the argument to [f] is.
...
Local Ltac path_forall_beta_t :=
  lazymatch goal with
    | [ |- context[@path_Π?H ?A ?B ?f ?g ?e] ]
      => let X := fresh in
         pose proof (eissect (@path_ΠH A B f g) e) as X;
           case X;
           generalize (@path_ΠH A B f g e);
           clear X; clear e;
           intro X; destruct X;
           simpl;
           unfold apD10;
...
Ltac infer_path_forall_recr_beta term :=
  let path_forall_recr_beta' :=
      match term with
        | @transport _ (λx, @?P0 x) _ _ (@path_Π?H ?A ?B ?f ?g ?e) _
          => constr:(λx0 P Px, @path_forall_recr_beta H A B x0 P f g e Px)
      end in
...
Ltac transport_path_forall_hammer_helper :=
  let term := match goal with
                | |- context[@transport ?At (λx, @?Bt x) ?ft ?gt (@path_Π?H ?A ?B ?f ?g ?e) ?Px]
                  => constr:(@transport At Bt ft gt (@path_ΠH A B f g e) Px)
              end in
