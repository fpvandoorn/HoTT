...
/- The record [is_equiv] has four components, so [issig4] can prove that it is equivalent to an iterated Sigma-type. -/

definition issig_isequiv {A B : Type} (f : A → B) :
  Σg:B->A, Σr:Sect g f, Σs:Sect f g, Πx : A, r (f x) = ap f (s x) 
  ≃ is_equiv f.
/-begin
  issig (is_equiv.mk A B f) (@equiv_inv A B f) (@retr A B f)
        (@sect A B f) (@adj A B f).
end-/
