...
definition ap10_path_pi {A B : Type} (f g : A → B) (h : f ∼ g)
  : ap10 (path_pi f g h) ∼ h :=
     apD10_path_pi f g h.
...
/-begin
  ...
  exact (apD10_path_pi f g h u ⬝ (concat_1p _)⁻¹).
end-/
...
definition isequiv_functor_arrow [instance] [H : is_equiv B A f] [H : is_equiv C D g]
  : is_equiv (functor_arrow f g) | 1000 :=
     @isequiv_functor_pi _ A (λ_, C) B (λ_, D)
     _ _ _ _.

definition equiv_functor_arrow [H : is_equiv B A f] [H : is_equiv C D g]
  : (A → C) ≃ (B → D) :=
     @equiv_functor_pi _ A (λ_, C) B (λ_, D)
  f _ (λ_, g) _.
