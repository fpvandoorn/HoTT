...
definition isequiv_path_unit [instance] (z z' : unit) : is_equiv (path_unit_uncurried z z') | 0.
  refine (is_equiv.mk _ _ (path_unit_uncurried z z') (λ_, star)
    (λp:z=z',
      match p in (_ = z') return (path_unit_uncurried z z' star = p) with
        | refl => match z as z return (path_unit_uncurried z z star = 1) with
                    | star => 1
                  end
      end)
...
definition isequiv_unit_ind [instance] [H : funext] (A : Type) : is_equiv (@Unit_ind (λ_, A)) | 0 :=
     isequiv_adjointify _
  (λf : unit → A, f star)
  (λf : unit → A, path_pi (@Unit_ind (λ_, A) (f star)) f
...
definition isequiv_unit_coind [instance] [H : funext] (A : Type) : is_equiv (@unit_coind A) | 0 :=
     isequiv_adjointify _
  (λf, star)
  _ _.
/-begin
...
/-begin
  refine (equiv.mk _ _
    ...
    (is_equiv.mk _ _ _
      ...
      (λx : A, contr x) _)).
  intro x. apply symmetry. apply ap_const.
end-/
