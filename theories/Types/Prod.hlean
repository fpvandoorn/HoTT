...
: is_equiv (path_prod_uncurried z z') | 0 :=
     is_equiv.mk
       _ _ _
...
definition isequiv_functor_prod [instance] [H : is_equiv A A' f] [H : is_equiv B B' g]
: is_equiv (functor_prod f g) | 1000 :=
     is_equiv.mk
       _ _ (functor_prod f g) (functor_prod f⁻¹ g⁻¹)
       (λz, path_prod' (retr f (pr1 z)) (retr g (pr2 z)) ⬝ eta_prod z)
       (λw, path_prod' (sect f (pr1 w)) (sect g (pr2 w)) ⬝ eta_prod w)
       _.
/-begin
  intros [a b]; simpl.
  unfold path_prod'.
...
definition equiv_functor_prod [H : is_equiv A A' f] [H : is_equiv B B' g]
  : A × B ≃ A' × B'.
/-begin
...
: is_equiv (@prod_coind_uncurried X A B) | 0 :=
     is_equiv.mk
       _ _
