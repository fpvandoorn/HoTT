...
: is_equiv (path_sum z z') | 0.
/-begin
  refine (is_equiv.mk _ _
                       (path_sum z z')
...
: transport (λa, P a + Q a) p z = match z with
     ...
     match p with refl => match z with inl _ => 1 | inr _ => 1 end end.
...
definition isequiv_functor_sum [instance] [H : is_equiv A A' f] [H : is_equiv B B' g]
: is_equiv (functor_sum f g) | 1000.
/-begin
  apply (isequiv_adjointify
           (functor_sum f g)
           (functor_sum f⁻¹ g⁻¹));
  [ intros [?|?]; simpl; apply ap; apply retr
  | intros [?|?]; simpl; apply ap; apply sect ].
end-/

definition equiv_functor_sum [H : is_equiv A A' f] [H : is_equiv B B' g]
: A + B ≃ A' + B' :=
...
: is_equiv (sum_ind_uncurried P) | 0.
/-begin
  apply (isequiv_adjointify
           (sum_ind_uncurried P)
...
definition isequiv_sigT_of_sum [instance] A B : is_equiv (@sigT_of_sum A B) | 0.
/-begin
...
definition isequiv_sum_of_sigT [instance] A B : is_equiv (sum_of_sigT A B) :=
