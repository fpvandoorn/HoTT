...
Class Univalence.
Axiom isequiv_equiv_path : Π[H : Univalence] (A B : Type), IsEquiv (equiv_path A B).
...
/-begin
  ...
  exact (ap (λs, transport idmap s z) (eissect _ p)).
end-/
...
definition transport_path_universe_equiv_path
  ...
  : transport_path_universe (equiv_path A B p) z =
    (ap (λs, transport idmap s z) (eissect _ p)) :=
...
/-begin
  revert f. equiv_intro (equiv_path A B) p.
  exact (ap (λs, transport idmap s z) (inverse2 (eissect _ p))).
end-/
...
definition transport_path_universe_V_equiv_path [H : Funext]
  ...
  : transport_path_universe_V (equiv_path A B p) z =
    ap (λs, transport idmap s z) (inverse2 (eissect _ p)) :=
     equiv_ind_comp _ _ _.
