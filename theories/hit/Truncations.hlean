  ...
  section TruncationModality
    ...
    : is_trunc n A <-> is_equiv (@tr n A) :=
       inO_iff_isequiv_to_O n A.

    definition isequiv_tr [instance] A [H : is_trunc n A] : is_equiv (@tr n A) :=
    ...
    definition isequiv_Trunc_functor {X Y} (f : X → Y) [H : is_equiv _ _ f]
    : is_equiv (Trunc_functor f) :=
