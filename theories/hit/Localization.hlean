...
| loc : X → Localize X
| islocal_localize : Πi, is_equiv (λ(g : T i → X), g ∘ f i).
>>

This is not a valid HIT by the usual rules, but if we expand out the definition of [is_equiv] and apply [path_sigma] and [path_forall], then it becomes one.  We get a simpler definition (no 2-path constructors) if we do this with [BiInv] rather than [is_equiv]:
...
/-begin
  ...
  - intros p q p' q'.
    ...
    apply ap, apply symmetry. eisretr.
end-/
  ...
  - intros g g'.
    ...
    + intros b.
      ...
      × refine (transport2 (D b) (p := 1) _ _).
        ...
        apply symmetry. refine ((pr1 (pr2 (ext 2) _ _) (λa', 1)).2 a).
      × intros a; simpl.
        refine (_ @
          ap (transport (D (f a)) ((pr1 (ext n.+1) g).2 a)⁻¹) (g' a)
          ⬝ _);
        [ apply symmetry. by apply apD
        | by apply apD ].
    + intros a; simpl.
      ...
      match goal with
          ...
          => refine (ap (transport P p) ((ap_pp f q r)⁻¹ @@ (refl s)) ⬝ _)
      end.
      ...
      × rewrite ap_V, concat_pp_p.
        do 2 apply moveR_Vp.
        rewrite concat_p1.
        apply symmetry. apply transport_pV_ap.
      × rewrite !concat_pp_p.
...
Module Localization_ReflectiveSubuniverses <: ReflectiveSubuniverses.
  ...
  definition inO_equiv_inO_internal :
    Π(O : ReflectiveSubuniverse@{u a}) (T U : Type@{i}),
      inO_internal@{u a i} ∘ T → Πf : T → U, is_equiv f → inO_internal@{u a i} ∘ U :=
