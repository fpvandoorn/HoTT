...
/- This could be implemented using the pushouts in /hit/Pushout.v, where [f] and [g] are [(fst ∘ dpr1)] and [(snd ∘ dpr1)], with domain Σ(a,b) : A × B, R a b. However, these pushouts weren't implemented when I started this work, and doing it this way is closer to exercise 10.11 of the HoTT book -/
...
Lemma bisimulation_equiv_id : Πu v : V, (u ≈ v) ≃ (u ~~ v).
/-begin
  intros u v.
...
Lemma eq_img_untrunc : (Πa : Au, Σa' : Au', mu' a' ≈ mu a)
...
Let e : Au → Au' := λa, dpr1 (fst eq_img_untrunc a).
Let inv_e : Au' → Au := λa', dpr1 (snd eq_img_untrunc a').
...
/-begin
  ...
  exact (dpr2 (fst eq_img_untrunc (inv_e a'))).
  exact (dpr2 (snd eq_img_untrunc a')).
end-/
...
/-begin
  ...
  apply (isinj_embedding mu mono).
  transitivity (mu' (e a)).
  exact (dpr2 (snd eq_img_untrunc (e a))).
  exact (dpr2 (fst eq_img_untrunc a)).
end-/
...
Lemma mu_eq_mu' : transport (λA : Type, A → V) path⁻¹ mu ≈ mu'.
/-begin
  ...
  transitivity (mu (inv_e a')).
  2: apply (dpr2 (snd eq_img_untrunc a')).
  refine (ap mu _).
...
Lemma monic_set_present_uniqueness : (Au; (mu; (h, mono, p))) ≈ (Au'; (mu'; (h', mono', p'))) :> ΣA : Type, Σm : A → V, IsHSet A × IsEmbedding m × (u ≈ set m).
/-begin
...
Lemma monic_set_present : Πu : V, exists (Au : Type) (m : Au → V),
...
definition is_hset_typeofmembers {u : V} : IsHSet ([u]) := fst (fst (dpr2 (dpr2 (monic_set_present u)))).

definition IsEmbedding_funcofmembers {u : V} : IsEmbedding func_of_members := snd (fst (dpr2 (dpr2 (monic_set_present u)))).

definition is_valid_presentation (u : V) : u ≈ set func_of_members := snd (dpr2 (dpr2 (monic_set_present u))).


/- Lemmas 10.5.8 (i) & (vii), we put them here because they are useful later -/
Lemma extensionality : Π{x y : V}, (x ⊆ y × y ⊆ x) <-> x ≈ y.
/-begin
...
definition V_cart_prod (a : V) (b : V) : V :=
   set (λx : [a] × [b], [func_of_members (fst x), func_of_members (snd x)]).
...
Lemma not_mem_Vempty : Πx, ~ (x ∈ V_empty).
Proof.
  intros x Hx. generalize Hx; apply Trunc_rec.
  intros [ff _]. exact ff.
Qed.

Lemma pairing : Πu v, hexists (λw, Πx, x ∈ w <-> hor (x ≈ u) (x ≈ v)).
Proof.
...
Lemma infinity : (V_empty ∈ V_omega) × (Πx, x ∈ V_omega → V_succ x ∈ V_omega).
Proof.
...
Lemma union : Πv, hexists (λw, Πx, x ∈ w <-> hexists (λu, x ∈ u × u ∈ v)).
Proof.
...
Lemma function : Πu v, hexists (λw, Πx, x ∈ w <-> V_is_func u v x).
Proof.
...
Lemma separation (C : V → hProp) : Πa : V,
