...
definition succ_pos_injective {z w : Pos} (p : succ_pos z ≈ succ_pos w) : z ≈ w :=
     transport (λs, z ≈ (match s with one => w | succ_pos a => a end)) p (idpath z).
...
definition neg_injective {z w : Pos} (p : neg z ≈ neg w) : z ≈ w :=
     transport (λs, z ≈ (match s with neg a => a | zero => w | pos a => w end)) p (idpath z).

definition pos_injective {z w : Pos} (p : pos z ≈ pos w) : z ≈ w :=
     transport (λs, z ≈ (match s with neg a => w | zero => w | pos a => a end)) p (idpath z).

definition neg_neq_zero {z : Pos} : ~ (neg z ≈ zero) :=
     λp, transport (λs, match s with neg a => z ≈ a | zero => Empty | pos _ => Empty end) p (idpath z).

definition pos_neq_zero {z : Pos} : ~ (pos z ≈ zero) :=
     λp, transport (λs, match s with pos a => z ≈ a | zero => Empty | neg _ => Empty end) p (idpath z).
...
/-begin
  ...
  exact (inr (neg_neq_zero ∘ symmetry _ _)).
  exact (inl 1).
  exact (inr (pos_neq_zero ∘ symmetry _ _)).
  exact (inr (neg_neq_pos ∘ symmetry _ _)).
...
definition isequiv_succ_int [instance] : IsEquiv succ_int | 0 :=
