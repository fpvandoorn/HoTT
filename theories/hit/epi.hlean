...
section cones
  ...
  /-begin
    ...
    pose (l := ⟨tr ∘ push ∘ inl, refl⟩ : tot).
    pose (r := (@const B (setcone f) (setcone_point _); (ap (λf, @tr 0 _ ∘ f) (path_pi _ _ alpha1))) : tot).
    subst tot.
...
section isepi_issurj
  ...
  /-begin
    ...
    pose (contr_inhabited_hprop (fib (f x)) (tr ⟨x, refl⟩)) as i.
...
definition isepi_isequiv X Y (f : X → Y) [H : is_equiv _ _ f]
: isepi f.
Proof.
  intros ? g h H'.
  apply ap10 in H'.
