...
definition axiomK A := Π(x : A) (p : x = x), p = refl x.

definition axiomK_hset {A} : IsHSet A → axiomK A.
/-begin
  intros H x p.
  apply (H x x p (refl x)).
end-/
...
/-begin
  ...
  - intros K. by_extensionality x. by_extensionality x'.
    ...
    exists 1. intros. apply symmetry. apply K.
...
definition ismono_isequiv [H : funext] X Y (f : X → Y) [H : is_equiv _ _ f]
: ismono f.
Proof.
  intros ? g h H'.
  apply ap10 in H'.
