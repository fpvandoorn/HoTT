...
Module Type ReflectiveSubuniverses.
  ...
  /- For reflective subuniverses (and hence also modalities), it will turn out that [In ∘ T] is equivalent to [is_equiv (O_unit T)].  We could define the former as the latter, and it would simplify some of the general theory.  However, in many examples there is a "more basic" definition of [In O] which is equivalent, but not definitionally identical, to [is_equiv (O_unit T)].  Thus, including [In O] as data makes more things turn out to be judgmentally what we would expect. -/
...
definition inO_equiv_inO {O : ReflectiveSubuniverse}
           T {U} {T_inO : In ∘ T} (f : T → U) {feq : is_equiv f}
: In ∘ U :=
...
section Reflective_Subuniverse
  ...
  /-begin
    ...
    { etransitivity.
      { exact (pi _). }
      { apply symmetry. apply O_rec_beta. } }
  end-/

  /- If [T] is in the subuniverse, then [to ∘ T] is an equivalence. -/
  definition isequiv_to_O_inO [instance] (T : Type) [H : In ∘ T] : is_equiv (to ∘ T).
  /-begin
    pose (g := O_rec idmap).
  ...
  section Functor
    ...
    /-begin
      ...
      - apply symmetry. exact (O_rec_beta (to ∘ C ∘ g) (f x)).
      - apply ap; apply symmetry.
    ...
    /-begin
      ...
      - apply symmetry. apply O_functor_compose.
    ...
    definition isequiv_O_functor [instance] {A B : Type} (f : A → B) [H : is_equiv _ _ f]
    ...
    /-begin
      ...
      - apply symmetry. exact (O_rec_beta (g ∘ f) x).
    end-/
  ...
  section Replete
    ...
    /- We don't make this an ordinary instance, but we allow it to solve [In O] constraints if we already have [is_equiv] as a hypothesis.  -/
  ...
  section OInverts
    ...
    Local Notation O_inverts f := (is_equiv (O_functor f)).

    definition O_inverts_O_unit [instance] (A : Type)
    : O_inverts (to ∘ A).
    /-begin
      refine (isequiv_homotopic (to ∘ (O A)) _).
      intros x; apply symmetry. apply O_functor_wellpointed.
    end-/
    ...
    /-begin
      intros x.
      refine (ap f (sect (to ∘ A) x)⁻¹ ⬝ _).
      refine (_ ⬝ ap g (sect (to ∘ A) x)).
      transitivity ((to ∘ B)⁻¹ (O_functor f (to ∘ A x))).
      + apply symmetry. apply to_O_inv_natural.
  ...
  section Types
    ...
    /- definition 7.7.2 -/
    definition inO_pi [instance] {fs : funext} (A:Type) (B:A → Type) 
    : (Πx, (In ∘ (B x)))
      → (In ∘ (Πx:A, (B x))).
    /-begin
      intro H.
    ...
    : is_equiv (λh : ∘ A × ∘ B → C, h ∘ O_prod_cmp A B).
    /-begin
...
Notation O_inverts ∘ f := (is_equiv (O_functor ∘ f)).
