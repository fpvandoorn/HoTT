...
section swap_functor
  ...
  : (swap C D) ((swap D C) c)
    = c :=
       match c with type_inl _ => refl | type_inr _ => refl end.

  definition swap_involutive [H : funext] C D
  : swap C D ∘ swap D C = 1.
  Proof.
    path_functor.
    exists (path_pi _ _ swap_involutive_helper).
    repeat (apply (@path_pi _); intro).
    repeat match goal with
               | [ |- context[transport (λx, Πy, @?C x y) ?p ?f ?x] ]
                 => simpl rewrite (@transport_pi_constant _ _ C _ _ p f x)
           end.
