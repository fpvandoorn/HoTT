...
>>
  ...
  /- Said proof respects [refl] -/
  definition path_functor_uncurried_idpath F
  : @path_functor_uncurried F F ⟨refl, refl⟩ = idpath.
  Proof.
    destruct F; simpl in *.
    rewrite !(contr refl).
    reflexivity.
  ...
  : F = G.
  Proof.
    refine (path_functor F G (path_pi _ _ HO) _).
  ...
  : is_equiv (@path_functor_uncurried F G).
  /-begin
  ...
  : is_equiv (@path_path_functor_uncurried F G p q).
  /-begin
    unfold path_path_functor_uncurried.
    /- N.B. [exact _] is super-slow here.  Not sure why. -/
    repeat match goal with
             | [ |- is_equiv (_ ∘ _) ] => eapply @isequiv_compose
             | [ |- is_equiv (_⁻¹) ] => eapply @isequiv_inverse
             | [ |- is_equiv (path_sigma_uncurried _ _ _) ] => eapply @isequiv_path_sigma
           end.
  end-/
