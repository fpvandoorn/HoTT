  ...
  match goal with
    ...
    | [ |- context[contr (center ?x)] ]
      => progress let H := fresh in
                  assert (H : refl = contr (center x)) by exact (center _);
                    destruct H
  end.
...
Ltac exp_laws_handle_transport' :=
  idtac;
  match goal with
    | _ => progress rewrite ?transport_pi_constant, ?path_pi_2_beta, ?transport_const, ?path_functor_uncurried_fst, ?transport_path_prod
    ...
    | _ => progress transport_path_pi_hammer
