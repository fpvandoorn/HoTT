...
definition fcontr_isequiv `(f : A → B)
  : IsEquiv f → (Πb:B, is_contr Σa : A, f a ≈ b).
/-begin
  intros ? b.  exists ⟨f⁻¹ b , eisretr f b⟩.  intros [a p].
  refine (path_sigma' _ ((ap f⁻¹ p)⁻¹ ⬝ eissect f a) _).
...
definition contr_sect_equiv `(f : A → B) [H : IsEquiv A B f]
...
definition contr_retr_equiv `(f : A → B) [H : IsEquiv A B f]
...
Local definition hprop_isequiv `(f : A → B) : is_hprop (IsEquiv f).
/-begin
  ...
  refine (@contr_Π_ _ _ _); intros a.
...
/-begin
  ...
  refine (equiv_compose' _ (equiv_functor_Πidmap
